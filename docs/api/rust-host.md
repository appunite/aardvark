...
> [!NOTE]
> The workspace ships prebuilt PIC-friendly V8 142.0.0 archives built from the upstream tag with `v8_monolithic=true` and `v8_monolithic_for_shared_library=true`. `.cargo/config.toml` sets `RUSTY_V8_MIRROR` to our GitHub release (`https://github.com/appunite/aardvark/releases/tag/v142.0.0`) so `cargo build` links against those artifacts automatically. Hosts that produce `cdylib` outputs (e.g. NIFs) can rely on this default or override `RUSTY_V8_MIRROR` / `RUSTY_V8_ARCHIVE` to point at their own build. Treat this mirror as experimental and keep the team posted if your linker still objects.
