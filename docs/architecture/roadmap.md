...
- **Observability** – extend diagnostics with per-request timestamps, queue
  metrics beyond the current P50/P95 fields, and tracing integration helpers for
  non-Rust hosts.
...
- **Async host adapters** – RawCtx adapters that expose async host functions with capability gates.
...
Have an idea or pressing gap? Open an issue or update this roadmap with the
specific code path, workload, and verification target.
